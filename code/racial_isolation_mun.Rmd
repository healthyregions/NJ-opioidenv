...
mun.isob <- mun_sum %>%
  mutate(iso.b=(nhblack/mun_blk * nhblack/total)) %>%
  group_by(SSN) %>%
  summarise(iso.b= sum(iso.b, na.rm=T))
...
# convert to sf
nj_seg_dat <- st_as_sf(nj_seg_dat)
```
...
mun.iso.h <- mun_sum %>%
  mutate(iso.h = (hisp/mun_hisp * hisp/total))%>%
  group_by(SSN)%>%
  summarise(iso.h= sum(iso.h, na.rm=T))
...
# convert to sf 
nj_seg_dat_h <- st_as_sf(nj_seg_dat_h)
...
mun.dis.a <- mun_sum %>%
  mutate(d.wa = abs(nhwhite/mun_wht - nhasian/mun_asian))%>%
  group_by(SSN)%>%
  summarise(dissim.a = .5*sum(d.wa, na.rm=T))

# Interaction Index - Asian
## Next is the interaction index for Hispanics and whites. In the calculation first population is minority population, second is non-minority population. The formula is: 
# Interaction = ∑i ai/A ∗ wi/ti

mun.int.a <- mun_sum %>%
  mutate(int.aw = (nhasian/mun_asian * nhwhite/total)) %>%
  group_by(SSN)%>%
...
mun.iso.a <- mun_sum %>%
  mutate(iso.a = (nhasian/mun_asian * nhasian/total))%>%
  group_by(SSN)%>%
  summarise(iso.a = sum(iso.a, na.rm = T))

nj_seg_a <- list(mun.dis.a, mun.int.a, mun.iso.a) %>% 
  reduce(left_join, by="SSN")

# merge with the shapefile
nj_seg_dat_a <- geo_join(json, nj_seg_a, by_sp="SSN", by_df="SSN")

# convert to sf 
nj_seg_dat_a <- st_as_sf(nj_seg_dat_a)
```
