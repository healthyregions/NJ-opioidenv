...
mun_sum <- tr_mun %>%
  group_by(Place.Name, SSN) %>%
  summarise(mun_total=sum(total), mun_wht=sum(nhwhite), mun_blk=sum(nhblack), mun_asian=sum(nhasian), mun_oth=sum(nhother), mun_hisp=sum(hisp))

test <- left_join(tr_mun, mun_sum, by = "Place.Name")
colSums(is.na(test))
test <- tr_mun %>%
  group_by(Place.Name, SSN) %>%
  mutate(summarise(mun_total=sum(total), mun_wht=sum(nhwhite), mun_blk=sum(nhblack), mun_asian=sum(nhasian), mun_oth=sum(nhother), mun_hisp=sum(hisp)))

muni_trdat <- left_join(x = trdat, y = mun_sum, by = "GEOID")
head(muni_trdat)

merged <- left_join(x = muni_trdat, y = interpolation, by = c("GEOID" = "TRACTID"))
