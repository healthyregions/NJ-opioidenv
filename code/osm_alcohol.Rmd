...
nj_liquorstores_sum <- nj_bounds %>% 
  mutate(tract_area = st_area(geometry)) %>%
  st_join(nj_liquorstores, left = FALSE) %>%
  group_by(GEOID) %>% 
  summarize(tract_area = unique(tract_area), 
            population = unique(POPULATION),
            n_liquorstores = n(), 
...
```{r}
nj_bars_liquorstores <- nj_bounds %>% 
  left_join(nj_bars_sum%>%st_drop_geometry(), by = "GEOID")  %>% 
  left_join(nj_liquorstores_sum %>%st_drop_geometry()%>%subset(select = -c(tract_area, population)), by = "GEOID")%>%
  subset(select = -c(TRACTCE, COUNTYFP, STATEFP, NAME, NAMELSAD, MTFCC, FUNCSTAT, ALAND, AWATER, INTPTLAT, INTPTLON))
```
...
```
```{r}
plot(nj_bars_liquorstores['liquorstores_per_capita'])
```

```{r}
st_write(nj_bars_liquorstores, "nj_bars_liquorstores.geojson")
```
