...
cw_merge <- merge(residential_environment, cw, by = "GEOID")
...
# merge crosswalk and distance matrix
cw_merge2 <- merge(residential_environment, cw, by = "GEOID")
...
rent$percent_renter <- rent$occupied_renter / rent$occupied_units

final2 <- aggregate(rent$percent_renter, by=list(SSN=new$SSN), FUN=sum)
final2 <- rename(final2, percent_renter = x)
...
final <- merge(final, final2, by = "SSN")
